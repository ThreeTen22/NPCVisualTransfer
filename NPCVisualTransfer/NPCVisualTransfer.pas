...
const
  ...
  ScriptName = 'NPC Visual Transferer';
var 
...
var
  ...
  s, s2: string;
begin
  DateTimeToString(s2, 'm d y h m',t);
  s := geev(DestFL,'EDID');
...
begin
  ...
  temp.Add(' ');
  DateTimeToString(fdate, 'm d y h m',t);
  CopyDirectory(TempPath, DataPath+moDataFolder+'\'+fdate+'\',temp,true);
  temp.free;
end;
...
begin
  ...
  if bHasAssets then begin
    ...
    DeleteFile(TempPath+lMeshPath+dFileName+'\'+dHex+'.nif');
    DeleteFile(TempPath+lTexPath+dFileName+'\'+dHex+'.dds');
    end;
  end;
...
begin
  ...
  //Make Backups of what we are overriding.  Only worried about overrides
    ...
    wCopyFile(tXferPath+mdFile,TempPath+'OverwrittenFaceGens\'+mdFile, true);
    wCopyFile(tXferPath+tdFile,TempPath+'OverwrittenFaceGens\'+tdFile, true);
...
begin
  ...
  if cFilePath = '' then begin
    ...
    if moButton = mrYes then begin
      ...
      MessageDlg('Note:  This script IS functional with Nexus Mod Manager and Unmanaged games.  But I will be DIRECTLY modifying textures and meshes from the data file with very limited capability of controlling what will happen to the assets of the NPCs I manage after patching.  Therefore it will be on you to remember what NPCs I have modified and make sure their FaceGenData and assets do not get overwritten.  I highly highly highly suggest getting mod organizer so all these issues and the problimatic issue of overwriting files in general will be moot.', mtError, [mbOk], 0);
