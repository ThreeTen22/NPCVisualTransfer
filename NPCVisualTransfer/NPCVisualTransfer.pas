{
  ...
  ************************************************************************
         ...
         IF YOU CAN READ THIS AND ARE USING THE BAT FILE OR THE .TES5EDIT FILE TO RUN THIS PATCH
                          THEN YOU NEED TO UPDATE TES5EDIT!
...
begin
  ...
  if bHasAssets then begin
    ...
    if not DeleteFile(delFile) then begin
      DeleteFile(PChar(delFile));
    end;
    delFile := xferPath+moDataFolder+'\'+lTexPath+dFileName+'\'+dHex+'.dds';
    if not DeleteFile(delFile) then begin
      DeleteFile(PChar(delFile));
    end;
    if not bUsingMO then begin
...
begin
  ...
  zz := slNewElements.IndexOf(HexFormID(DestNPC));
  if zz > (-1) then slNewElements.Delete(zz);
...
begin
  ...
  AddMessage('Source: '+ShortName(SourceNPC));
  AddMessage('Dest: '+ ShortName(DestNPC));
...
function RecordByFile(e, f: IInterface): IInterface;
var
  i: Integer;
  ovr: IInterface;
begin
